...
def inverse_softmax(xj, num_classes):
    return np.log(199*(num_classes - 1)) + xj
...
class ImageClassifier(torch.nn.Module):
    ...
    def __init__(self, P, model_feature_extractor=None, model_linear_classifier=None):
        ...
        elif self.arch == 'effnet':
            if P['use_pretrained']:
                print('feature extractor: efficientnet_b7 pretrained')
                effnet = efficientnet_b7(pretrained=True)
            else:
                print('feature extractor: randomly initialized efficientnet_b7')
                effnet = efficientnet_b7(pretrained=False)

            feature_extractor = torch.nn.Sequential(*list(effnet.children())[:-1])
            linear_classifier = torch.nn.Sequential(*list(effnet.children())[-1])

            f = False if P['freeze_feature_extractor'] else True
            print(f"feature extractor {'trainable' if f else 'frozen'}")
            for param in feature_extractor.parameters():
                param.requires_grad = f
                    
            linear_classifier[1] = torch.nn.Linear(2560, P['num_classes'], bias=True)
...
class LabelEstimator(torch.nn.Module):
    ...
    def __init__(self, P, observed_label_matrix, estimated_labels):
        ...
        if P['activate_function'] == 'sigmoid':
            ...
            if estimated_labels is None:
                # initialize unobserved and observed negative labels:
                a = 2 * torch.rand(1,1).item() - 1
                param_mtx = a * torch.ones(num_examples, P['num_classes'])
                
                # initialize observed positive labels:
                init_logit_pos = inverse_softmax(a, P['num_classes'])
                idx_pos = torch.from_numpy((observed_label_matrix == 1).astype(bool))
                param_mtx[idx_pos] = init_logit_pos
            else:
                param_mtx = inverse_softmax(torch.FloatTensor(estimated_labels), P['num_classes'])
        
        self.activate_function = P['activate_function'] ###
        self.logits = torch.nn.Parameter(param_mtx)
        
    def get_estimated_labels(self):
        with torch.set_grad_enabled(False):
            estimated_labels = torch.sigmoid(self.logits) if self.activate_function == 'sigmoid' else torch.softmax(self.logits,dim=1) ###
        estimated_labels = estimated_labels.clone().detach().cpu().numpy()
        return estimated_labels
    
    def forward(self, indices):
        x = self.logits[indices, :]
        x = torch.sigmoid(x) if self.activate_function == 'sigmoid' else torch.softmax(x, dim=1) ###
        return x
