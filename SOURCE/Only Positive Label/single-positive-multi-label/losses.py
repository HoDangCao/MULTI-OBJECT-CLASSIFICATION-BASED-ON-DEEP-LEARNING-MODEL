...
loss functions
'''
def loss_squared_hinge(batch, P, Z):
    # unpack:
    preds = batch['preds']
    ...
    # compute loss:
    loss_mtx = 1 - true_labels*preds
    loss_mtx[loss_mtx < 0] = 0
    loss_mtx = loss_mtx**2
    reg_loss = None
    return loss_mtx, reg_loss

# def _huber_loss(y_pred, y, delta=1.0):
...
# def _focal_loss(p, gamma=2, alpha = 0.25):
#     loss = -alpha*(1-p)**gamma*np.log(p)
#     return loss

def loss_focal(batch, P, Z):
    gamma = 2
    alpha = 0.25
    ...
    # compute loss:
    loss_mtx = torch.zeros_like(true_labels)
    loss_mtx[true_labels == 1] = -alpha*(1-preds[true_labels == 1])**gamma*neg_log(preds[true_labels == 1])
    loss_mtx[true_labels == 0] = -(1-alpha)*(preds[true_labels == 0])**gamma*neg_log(1.0 - preds[true_labels == 0])
    reg_loss = None
    return loss_mtx, reg_loss

def loss_asymetric(batch, P, Z):
    pos_gamma = 0
    neg_gamma = 4
    m = 0.05
    ...
    # compute loss:
    loss_mtx = torch.zeros_like(true_labels)
    loss_mtx[true_labels == 1] = -(1-preds[true_labels == 1])**pos_gamma*neg_log(preds[true_labels == 1])
    ma_neg = preds[true_labels == 0] - m
    ma_neg[ma_neg < 0] = 0
    loss_mtx[true_labels == 0] = -ma_neg**neg_gamma*neg_log(1.0 - ma_neg)
    reg_loss = None
    return loss_mtx, reg_loss

# kl_loss = y_true * tf.math.log(y_true / y_pred) + (1 - y_true) * tf.math.log((1 - y_true) / (1 - y_pred))
def loss_KL(batch, P, Z):
    # unpack:
    preds = batch['preds']
    true_labels = batch['label_vec_true']
    # input validation: 
    ...
    # compute loss:
    loss_mtx = torch.zeros_like(true_labels)
    loss_mtx[true_labels == 1] = -neg_log(1/preds[true_labels == 1])
    loss_mtx[true_labels == 0] = -neg_log(1/(1.0 - preds[true_labels == 0]))
    reg_loss = None
    return loss_mtx, reg_loss
...
def loss_bce_ls(batch, P, Z):
    ...
    # compute loss:
    loss_mtx = torch.zeros_like(true_labels)
    loss_mtx[true_labels == 1] = (1.0 - P['ls_coef']) * neg_log(preds[true_labels == 1]) + P['ls_coef'] * neg_log(1.0 - preds[true_labels == 1])
    loss_mtx[true_labels == 0] = (1.0 - P['ls_coef']) * neg_log(1.0 - preds[true_labels == 0]) + P['ls_coef'] * neg_log(preds[true_labels == 0])
    reg_loss = None
    return loss_mtx, reg_loss
