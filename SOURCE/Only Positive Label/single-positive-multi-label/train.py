...
pp.add_argument('--loss', type=str, default='role', choices=['bce', 'bce_ls', 'iun', 'iu', 'pr', 'an', 'an_ls', 'wan', 'epr', 'role', 'fo', 'asl', 'hu', 'hi', 'kl'], required=False)
pp.add_argument('--train_mode', type=str, default='end_to_end', choices=['linear_fixed_features', 'end_to_end', 'linear_init'], required=False)
pp.add_argument('--arch', type=str, default='effnet', choices=['resnet50', 'effnet'], required=False)
args = pp.parse_args()
...
def run_train_phase(model, P, Z, logger, epoch, phase):
    ...
    for batch in Z['dataloaders'][phase]:
        ...
        batch['label_vec_true'] = batch['label_vec_true'].to(Z['device'], non_blocking=True) ###
        # forward pass: 
        Z['optimizer'].zero_grad()
        with torch.set_grad_enabled(True):
            # batch['logits'], batch['label_vec_est'] = model(batch)
            batch['logits'] = model.f(batch['image'])
            batch['preds'] = torch.sigmoid(batch['logits']) if P['activate_function'] == 'sigmoid' else torch.softmax(batch['logits'], dim=1) ###
...
def train(model, P, Z):
    ...
    for epoch in range(P['num_epochs']):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            #tensorboard log
            # writer.add_scalar(f'{phase}/{P["loss"]}', logger.logs['metrics'][phase][epoch]['loss'],epoch)
            # writer.add_scalar(f'{phase}/map_clean', logger.logs['metrics'][phase][epoch]['map_clean'],epoch)
            # writer.add_scalar(f'{phase}/map_observed', logger.logs['metrics'][phase][epoch]['map_observed'],epoch)
...
if __name__ == '__main__':
    ...
    lookup = {
        ...
        'linear_init_params': { # best learning rate and batch size for linear_fixed_features phase of linear_init
            ...
            'hu': {
                'coco': {'linear_init_lr': 1e-4, 'linear_init_bsize': 16},
            }
        }
    }
    ...
    P['loss'] = args.loss # bce, bce_ls, iun, iu, pr, an, an_ls, wan, epr, role | fo, hu, hi, kl
    P['train_mode'] = args.train_mode # linear_fixed_features, end_to_end, linear_init
    P['val_set_variant'] = 'observed' # clean, observed
    P['activate_function'] = 'sigmoid' # or softmax ###
    ...
    if P['loss'] in ['bce', 'bce_ls','fo', 'hu', 'hi', 'kl']:
