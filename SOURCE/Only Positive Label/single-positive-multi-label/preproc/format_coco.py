...
pp.add_argument('--load-path', type=str, default='../../fdata', help='Path to a directory containing a copy of the COCO dataset.')
pp.add_argument('--save-path', type=str, default='../../fdata', help='Path to output directory.')
args = pp.parse_args()
...
for split in ['train', 'val']:
    ...
    with open(os.path.join(args.load_path, split, 'annotations.json'), 'r') as f:
        D = json.load(f)
    ...
    image_ids = np.array(['{}/images/{}.jpg'.format(split, str(int(x)).zfill(6)) for x in image_ids])
