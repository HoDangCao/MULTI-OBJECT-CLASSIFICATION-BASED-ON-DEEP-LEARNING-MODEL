...
class Backbone1(nn.Module):
    def __init__(self):
        ...
        print('feature extractor: efficientnet_b0 pretrained')
        feature_extractor = models.efficientnet_b0(pretrained=True)
        
        feature_extractor.features[-1][0] = torch.nn.Conv2d(320, 2048, kernel_size=(1, 1), stride=(1, 1), bias=False)
        feature_extractor.features[-1][1] = torch.nn.BatchNorm2d(2048, eps=0.001, momentum=0.01, affine=True, track_running_stats=True)
...
# mnasnet0_75
class Backbone5(nn.Module):
    def __init__(self):
        super(Backbone5, self).__init__()
        print('feature extractor: mnasnet0_75')
       
        feature_extractor = models.mnasnet0_75(pretrained = True)

        feature_extractor.layers[-3] = torch.nn.Conv2d(240, 2048, kernel_size=(1, 1), stride=(1, 1), bias=False)
        feature_extractor.layers[-2] = torch.nn.BatchNorm2d(2048, eps=1e-05, momentum=0.00029999999999996696, affine=True, track_running_stats=True)
...
# Mobilenet_v2
class Backbone6(nn.Module):
    def __init__(self):
        super(Backbone6, self).__init__()
        print('feature extractor: mobilenet_v2')
       
        feature_extractor = models.mobilenet_v2(pretrained = True)

        feature_extractor.features[-1][0] = torch.nn.Conv2d(320, 2048, kernel_size=(1, 1), stride=(1, 1), bias=False)
        feature_extractor.features[-1][1] = torch.nn.BatchNorm2d(2048, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
...
class LabelEstimator(torch.nn.Module):
    def __init__(self, observed_label_matrix):
        ...
        param_mtx = q * (2 * torch.rand(num_examples, 323) - 1)
...
class Backbone2(torch.nn.Module):
    def __init__(self, train_obs,val_obs):
        super(Backbone2, self).__init__()
        
        self.f = Backbone1()
        self.g_train = LabelEstimator(train_obs)
        self.g_val = LabelEstimator(val_obs)

    def forward(self, batch):
        f_logits = self.f(batch['image'])
        g_preds_train = self.g_train(batch['idx'])
        g_preds_val = self.g_val(batch['idx'])
        return (f_logits, g_preds_train, g_preds_val)
